...
const clientPath = "/.local/share/mcpelauncher/games/com.mojang/";
...
const activeWorldFolderName = "RealtimeWorld";

const mcdir = useMinecraftDedicatedServer
  ? dedicatedServerPath
  : os.homedir() + clientPath;
    // (useMinecraftPreview
...
function copy_behavior_packs() {
  return gulp.src(["behavior_pack/**/*"]).pipe(gulp.dest("build/behavior_packs/" + bpfoldername));
}

function copy_resource_packs() {
  return gulp.src(["resource_pack/**/*"]).pipe(gulp.dest("build/resource_packs/" + bpfoldername));
}
...
function startServer(callbackFunction) {
  ...
  activeServer = spawn("./bedrock_server", [], {cwd:dedicatedServerPath, env:{LD_LIBRARY_PATH:"."}});
